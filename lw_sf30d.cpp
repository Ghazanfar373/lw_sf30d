...
uint32_t LW_SF30::i2cReadSelectDistanceOutputValues(int16_t report) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(29);    // ID
...
void LW_SF30::i2cWriteSelectDistanceOutputValues(uint32_t Value) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(29);    // ID
...
// them from the SF30/D.
// They are then printed in sequence on the serial port.
void LW_SF30::i2cReadDistanceOutputValues(int16_t report) {
  int16_t i = 0;
  uint16_t temp = 0;

  //Read from the SF30/D which outputs are selected for distance
...
void LW_SF30::i2cReadHardwareModel(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(0);    // ID
  Wire.endTransmission(false);
...
void LW_SF30::i2cReadHardwareVersion(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(1);    // ID
  Wire.endTransmission(false);
...
void LW_SF30::i2cReadFirmwareVersion(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(2);    // ID
  Wire.endTransmission(false);
...
void LW_SF30::i2cReadSerialNumber(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(3);    // ID
  Wire.endTransmission(false);
...
void LW_SF30::i2cReadToken(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(10);    // ID
  Wire.endTransmission(false);
...
void LW_SF30::i2cReadAPDTemp(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(55);    // ID
  Wire.endTransmission(false);
...
// This function reads the Token and saves it to the global variables
void LW_SF30::i2cReadSamplingRate(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(76);    // ID
  Wire.endTransmission(false);
...
// A new token must be requested and recieved and then passed to this
// function to send with the command
void LW_SF30::i2cWriteSaveParameters(uint16_t Token) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(12);    // ID
...
// A new token must be requested and recieved and then passed to this
// function to send with the command
void LW_SF30::i2cWriteResetCommand(uint16_t Token) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(14);    // ID
...
// This will change the Serial Baudrate
void LW_SF30::i2cWriteSerialBaudrate(uint8_t Rate) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(91);    // ID
...
// This will request the current I2C Address
void LW_SF30::i2cReadI2CAddress(int16_t report) {
  int16_t i = 0;
  Wire.beginTransmission(I2CAddress);
  Wire.write(92);    // ID
  Wire.endTransmission(false);
...
// This will change the I2C interface Address
void LW_SF30::i2cWriteI2CAddress(uint8_t Address) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(92);    // ID
...
// This will change the Sampling Rate
void LW_SF30::i2cWriteSamplingRate(uint8_t Rate) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(76);    // ID
...
void LW_SF30::i2cWriteMedianEnable(uint8_t En) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(86);    // ID
...
void LW_SF30::i2cWriteSmoothingEnable(uint8_t En) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(88);    // ID
...
void LW_SF30::i2cWriteMedianFilter(uint32_t Filter) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(87);    // ID
...
// This will change the Median Filter
void LW_SF30::i2cWriteSmoothingFilter(uint32_t Filter) {
  Wire.beginTransmission(I2CAddress);
  Wire.write(89);    // ID
